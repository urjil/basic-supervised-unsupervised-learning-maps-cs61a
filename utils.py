...
def key_of_min_value(d):
    ...
    """
    # BEGIN Question 0
    #return min(d.items(), key=lambda x: x)
    #return min(x[1] for x in d.items())
    #return min(d.items(), key= lambda x: x[1])
    return min(d.keys(), key= lambda x: d[x])

    #return min(d[x] for x in d.keys() 
    # END Question 0
...
def enumerate(s, start=0):
    ...
    #zip both together, take an arange from start till length of s 
    # END Question 0
...
def mean(s):
    ...
    for x in s:
        total=total+x
    average=float(total/len(s)) #total/len(s) # 
    return average
